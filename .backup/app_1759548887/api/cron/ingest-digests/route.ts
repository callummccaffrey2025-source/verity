...
export async function GET(req: Request) {
  ...
  try {
    const fileUrl = pathToFileURL(path.join(process.cwd(), 'scripts/ingest-aph-digests.mjs')).href;
    ...
    const fn =
      (mod as any)?.default && typeof (mod as any).default === 'function'
        ...
        : null;
    if (fn) await fn();
    else console.warn('[cron] digests module has no default/run');
    return NextResponse.json({ ok: true });
  } catch (err) {
    console.error('[cron] digests error', err);
