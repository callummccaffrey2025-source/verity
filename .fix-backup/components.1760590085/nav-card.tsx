  ...
  return (
    <Link href={href} className="card card-hover card-ring block p-5 group outline-none">
      <div className="flex items-start justify-between gap-4">
        ...
        <span className="text-neutral-100/80 group-hover:text-emerald-300 transition-transform group-hover:translate-x-0.5">
          <ArrowRight className="size-5" />
        </span>
      </div>
      {body && <p className="mt-2 text-sm text-neutral-100">{body}</p>}
    </Link>
  );
}
