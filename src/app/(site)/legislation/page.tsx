...
export default async function LegislationIndex(){
  const items = await getBills();
  return (
    <main className="mx-auto max-w-5xl px-6 py-10">
      <h1 className="text-3xl font-semibold mb-4">Legislation</h1>
      <div className="grid gap-4 md:grid-cols-2">
        {items.map(b=>(
          <a key={b.id} href={`/legislation/${b.id}`} className="rounded-xl border border-white/10 p-4 hover:border-emerald-400/40">
            <div className="font-medium">{b.title}</div>
            <div className="text-xs text-neutral-400">{(b.stage ?? "—")} · {(b.introduced ?? "—")}</div>
          </a>
        ))}
