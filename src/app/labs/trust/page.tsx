...
export default function Page(){
  ...
  return (
    <div className="p-6 space-y-4">
      ...
      <p className="text-zinc-300 text-sm">Rate reflects share of externally fact-checked statements rated accurate over time.</p>
      <SourceGuard receipts={receipts}>
        <div className="space-y-2">{items.map((i,idx)=>(
          <div key={idx} className="rounded-xl border border-zinc-800 p-4 flex justify-between">
            <span>{i.mp}</span><span className="text-zinc-400">{Math.round(i.truthRate*100)}%</span>
          </div>
        ))}</div>
      </SourceGuard>
    </div>
