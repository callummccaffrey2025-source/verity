...
export default async function News(){
  ...
  return (<div>
    ...
    <p className="mt-2 text-neutral-100">Side-by-side receipts. Ownership context. Bias at a glance.</p>
    <div className="mt-4 flex flex-wrap gap-2">
      {data.clusters.map(c=><a key={c} href={`/news/${encodeURIComponent(c)}`} className="px-3 py-1.5 rounded-full text-sm border border-zinc-800 bg-zinc-900/30 text-neutral-100 hover:border-emerald-700/60">{c}</a>)}
    </div>
