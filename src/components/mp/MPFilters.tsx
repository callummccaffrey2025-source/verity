...
export function MPFilters({ value, onChange }: { value: MPFilterState; onChange:(v:MPFilterState)=>void }) {
  ...
  return (
    <aside className="rounded-2xl border border-zinc-800 bg-zinc-900/40 p-4">
      ...
      <label className="mt-3 block text-sm text-zinc-300">Search
      ...
      <div className="mt-3 text-sm text-zinc-300">Party</div>
      <div className="mt-2 flex flex-wrap gap-2 text-xs">
        {['Labor','Liberal','National','Greens','Independent'].map(p=>(
          <button key={p} onClick={()=>set({ party: toggleArr(value.party,p) })}
                  className={`rounded-full border px-2.5 py-1 ${value.party.includes(p)?'border-emerald-700/60 bg-emerald-900/20 text-emerald-200':'border-zinc-700 bg-zinc-900 text-zinc-300'}`}>{p}</button>
        ))}
      </div>
      <div className="mt-3 text-sm text-zinc-300">Chamber</div>
      <div className="mt-2 flex flex-wrap gap-2 text-xs">
        {(['House','Senate'] as const).map(c=>(
          <button key={c} onClick={()=>set({ chamber: toggleArr(value.chamber,c) })}
                  className={`rounded-full border px-2.5 py-1 ${value.chamber.includes(c)?'border-emerald-700/60 bg-emerald-900/20 text-emerald-200':'border-zinc-700 bg-zinc-900 text-zinc-300'}`}>{c}</button>
        ))}
      </div>
      <label className="mt-3 block text-sm text-zinc-300">Minimum attendance
