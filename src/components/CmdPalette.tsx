...
export default function CmdPalette(){
  ...
  return(<div role="dialog" aria-modal="true" className="fixed inset-0 z-50 flex items-start justify-center pt-24 bg-black/60">
    <div className="w-[560px] max-w-[95vw] rounded-xl border border-zinc-800 bg-[#0a0f14] p-3">
      <div className="flex items-center gap-2 border-b border-zinc-800 pb-2"><Search size={18} className="text-zinc-500"/><input autoFocus value={q} onChange={e=>setQ(e.target.value)} placeholder="Jump to…" className="flex-1 bg-transparent outline-none text-sm text-zinc-200"/><kbd className="text-xs text-zinc-500">Esc</kbd></div>
