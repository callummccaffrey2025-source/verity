...
export default function SectionNavClient() {
  ...
  return (
    <nav aria-label="Profile sections"  className="tabs sticky top-0 z-10 backdrop-blur supports-[backdrop-filter]:bg-white/5 no-print">
      <div className="container-page mx-auto px-4 overflow-x-auto">
        <ul className="flex gap-2 py-2">
          {LINKS.map(link => (
            <li key={link.href}>
              <a
                href={link.href} aria-current={active === link.href ? "page" : undefined}
