...
export default function NewsCard({ s }:{ s:Story }){
  return(<article className="card rounded-2xl p-4">
    <div className="text-xs text-zinc-400">{s.cluster} · {new Date(s.published_at).toLocaleDateString("en-AU",{weekday:"short",hour:"2-digit",minute:"2-digit"})}</div>
    <h3 className="mt-2 text-[18px] font-semibold leading-snug">{s.title}</h3>
    {s.excerpt && <p className="mt-1 text-sm text-zinc-400">{s.excerpt}</p>}
    <div className="mt-3"><CoverageBar mix={s.coverageMix}/></div>
    <div className="mt-3 flex items-center justify-between gap-3">
      <div className="text-sm text-zinc-300"><Dot stance={s.primary.stance}/><span className="font-medium">{s.primary.name}</span>{s.primary.owner && <span className="text-zinc-500"> · {s.primary.owner}</span>}<span className="text-zinc-500"> · {timeAgo(s.published_at)}</span></div>
